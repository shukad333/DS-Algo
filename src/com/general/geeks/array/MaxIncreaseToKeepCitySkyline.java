 ...
 * Notes:
 * 
 * 1 < grid.length = grid[0].length <= 50. All heights grid[i][j] are in the
 * range [0, 100]. All buildings in grid[i][j] occupy the entire grid cell: that
	...
	public int maxIncreaseKeepingSkyline(int[][] grid) {
		...
		for (int i = 0; i < r; i++) {
			for (int j = 0; j < c; j++) {
				...
				resp += Math.min(maxRows[i], maxCols[j]) - grid[i][j];
