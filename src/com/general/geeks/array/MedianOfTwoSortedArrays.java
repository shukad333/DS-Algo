...
public class MedianOfTwoSortedArrays {
	...
	public static void main(String[] args) {
		...
		System.out.println(new MedianOfTwoSortedArrays().findMedianSortedArrays(new int[] {1}, new int[] {2,3}));
	...
	public double findMedianSortedArrays(int[] nums1, int[] nums2) {
		...
		int lo=0,high=2*N2; //smaller numer as in the explanation above
		while(lo<=high) {
			int mid2 = (lo+high)/2; //cut 1
			int mid1 = N1+N2-mid2;//cut 2
			
			double L1 = (mid1==0) ? Double.MIN_VALUE : nums1[(mid1-1)/2]; // (N-1)/2
			double L2 = (mid2==0) ? Double.MIN_VALUE : nums2[(mid2-1)/2]; // (N-1)/2
			double R1 = (mid1 == 2*N1) ?Double.MAX_VALUE : nums1[mid1/2];
			double R2 = (mid2==2*N2) ? Double.MAX_VALUE : nums2[mid2/2];
			
			if(L1>R2) lo = mid2+1; //A1 lower half is big , move C1 left or C2 right
			else if(L2>R1) high = mid2-1; //A2 lower half is big , move C2 left
