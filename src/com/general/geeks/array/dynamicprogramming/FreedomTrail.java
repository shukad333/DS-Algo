	...
	private int dfs(String ring , String key,int index , Map<String, Integer> map) {
		...
		for(int i=0;i<ring.length();i++) {
			...
			if(ring.charAt(i)==ch) {
				...
				String shiftedString  = ring.substring(i, ring.length()) + ring.substring(0, i);
				int steps = 1 + Math.min(i, ring.length()-i);//get the smallest - clockwise and anticlockwise
				steps+=dfs(shiftedString, key, index+1, map);
