...
private int removeSubBoxes(int[] boxes,int i,int j,int k , int[][][] dp) {
	...
	int resp = (k+1)*(k+1)+removeSubBoxes(boxes, i+1, j, 0, dp);
	
	for(int m=i+1;m<=j;m++) {
		if(boxes[i]==boxes[m])
		resp = Math.max(resp, removeSubBoxes(boxes, i+1, m-1, 0, dp)+removeSubBoxes(boxes, m, j, k+1, dp));
	}
	dp[i][j][k] = resp;
	return resp;
}
