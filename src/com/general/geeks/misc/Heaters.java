...
public class Heaters {
	...
	public int findRadius(int[] houses, int[] heaters) {
		...
		for (int house : houses) {
			...
			Integer upperBound = heaterSet.ceiling(house);
			Integer lowerBound = heaterSet.floor(house);

			radius = Math.max(radius, Math.min(upperBound == null ? Integer.MAX_VALUE : upperBound - house,
					lowerBound == null ? Integer.MAX_VALUE : house - lowerBound));
		}
