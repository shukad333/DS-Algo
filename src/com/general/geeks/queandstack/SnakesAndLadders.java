 ...
 * standard 6-sided die roll: ie., there are always at most 6 destinations.) If
 * S has a snake or ladder, you move to the destination of that snake or ladder.
 * Otherwise, you move to S. A board square on row r and column c has a "snake
 * or ladder" if board[r][c] != -1. The destination of that snake or ladder is
 * board[r][c].
 * 
 ...
 * [-1,35,-1,-1,13,-1], [-1,-1,-1,-1,-1,-1], [-1,15,-1,-1,-1,-1]] Output: 4
 * Explanation: At the beginning, you start at square 1 [at row 5, column 0].
 * You decide to move to square 2, and must take the ladder to square 15. You
 * then decide to move to square 17 (row 3, column 5), and must take the snake
 * to square 13. You then decide to move to square 14, and must take the ladder
 * to square 35. You then decide to move to square 36, ending the game. It can
 * be shown that you need at least 4 moves to reach the N*N-th square, so the
 * answer is 4. Note:
 * 
 * 2 <= board.length = board[0].length <= 20 board[i][j] is between 1 and N*N or
 * is equal to -1. The board square with number 1 has no snake or ladder. The
 * board square with number N*N has no snake or ladder.
 * 
 * 
 * 
 * @author skadavath
...
public class SnakesAndLadders {
	...
	public int snakesAndLadders(int[][] board) {
		...
		// store in a 1-D array

		int dir = 1, j = 0, n = board.length, i = n - 1, index = 0;
		int[] arr = new int[n * n];
		...
		int start = arr[0] > -1 ? arr[0] - 1 : 0, step = 0;
		queue.add(start);
